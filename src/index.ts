...
export class TableGrid {
   ...
   */
  public drawNavigation(): TableGrid {
    const self = this;
    ...
    const first = (_: MouseEvent) => this.scrollRows(0);
    const prev = (_: MouseEvent) => {
      this.scrollRows(-1);
      timer ? clearInterval(timer) : timer;
      timer = setInterval(() => self.scrollRows(-1), 250);
    };
    ...
    const last = (_: MouseEvent) => this.scrollRows(this._data.rows.length);
   ...
   */
  public scrollRows(n: number): TableGrid {
